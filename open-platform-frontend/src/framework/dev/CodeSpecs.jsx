    ...
    render() {
        return (
            <RightMarkDown source={['# 代码开发规范',
                ...
                '```',
                '### 接口名',
                'Java： I+实体名，例如：`IStudent`;',
                '接口的实现类+Impl结尾，例如：`StudentDaoImpl`, `StudentMockImpl`。',
                '### 变量名',
                ...
                '```',
                '### 其他命名',
                '* `controller`层的类名统一以**Controller**结尾。',
                '* `service`层的类名统一以**Service**结尾。',
                '* `dao`层的类名统一以**Dao**结尾。',
                '* 与数据库表对应的实体类统一以**Entity**结尾，`mybatis`文件统一以**Mapper**结尾。',
                ...
                '* 同一行不能有多个声明，不能有多个语句。',
                '* 提交代码前使用IDE的快捷键自动调整代码格式(his老代码不建议进行格式调整，否则无法对历史版本进行管理)。',
                ...
                '```',
                '### 缩小变量的作用域',
                '能用局部变量的，不要使用实例变量，能用实例变量的，不要使用类变量。变量的生存期越短，以为着它被误用的机会越小，', +
